...
if __name__=='__main__':
    ...
    parser.add_argument("--aux_lambda", type=str, default='50')
    ...
    parser.add_argument("--subsample_non_error", type=float, nargs='*') 
    ...
    parser.add_argument("--stageOne_T", type=int, default=20, help="T")
    ...
    parser.add_argument("--jtt", default=False, action="store_true", help="Two-M/One-M")
    parser.add_argument("--start_from_StageTwo", default=False, action="store_true", help="When Stage One has been runned")    
    parser.add_argument("--bertLstm", default=False, action="store_true", help="use bilstm model")
    ...
    stage_1_sub_exp_name = f"AUX1_epochs_{nepochs_Stage1}_lr_{lr_Stage1}_weightDecay_{wd_Stage1}_auxLambda_{aux_lambda}"
    ...
    + f" --aux_lambda {aux_lambda} --seed {args.seed} --loss {args.loss}" 
    ...
    if not args.jtt:
        ...
        + (f" --process_training --best_epoch {args.stageOne_T} --loadModel {args.stageOne_T}" if not args.ProcessWhole else "")
    ...
    num_processes = info_dict["n_epochs"] if args.ProcessWhole else 1 # Process All Epochs
    ...
    for final_epoch in range(int(num_processes)):
        ...
        final_epoch = final_epoch if args.ProcessWhole else args.stageOne_T # Process correct error set
        loadModel = final_epoch if args.ProcessWhole else args.stageOne_T # Either traverse or T
        ...
        for up_weight in up_weights:
            for subsample_nonerr_ratio in subsample_non_error:
                ...
                + f" --lr {lr_Stage1} --weight_decay {wd_Stage1} --up_weight {up_weight}" \
