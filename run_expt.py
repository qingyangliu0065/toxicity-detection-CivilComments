...
def main(args):
    ...
    print(f"Start memory usage: {mem_usage} MB")
    ...
    if args.method == "ProcessTrain":
        ...
        with open(os.path.join(args.log_dir_old, 'model_outputs/information.txt'), 'r') as f:
            for line in f:
                (k, v) = line.split()
                info_dict[k] = v
        # --lr 2e-05 --n_epochs 4 --weight_decay 0.0 
        args.lr = float(info_dict["lr"])
        args.n_epochs = int(info_dict["n_epochs"])
        # args.weight_decay = float(info_dict["weight_decay"])
    ...
    print("args.metadata_path: ", args.metadata_path)
    # Data
    ...
    if args.method == 'AUX2':
        ...
        # if args.dataset == "CUB":
        #     val_data = subsample_val_data(args, val_data, subsample_proportion=args.val_subsample)
        #     print("Subsample validation set: ", args.val_subsample)
        # else:
        #     val_data = subsample_val_data(args, val_data, subsample_proportion=args.val_subsample)
        #     print("Subsample validation set: ", args.val_subsample)
        
        # if args.dataset == "CUB" and args.control_val_group:
        #     val_data = val_fixed_size(args, val_data, group_size=[args.val_group0, args.val_group1,
        #                                                           args.val_group2, args.val_group3])
        #     print(f"Controlled validation set: group 0 {args.val_group0}; group 1 {args.val_group1}; group 2 {args.val_group2}; group 3 {args.val_group3}")
        # else:
        #     if args.dataset == "CUB":
        #         val_data = subsample_val_data(args, val_data, subsample_proportion=args.val_subsample)
        #         print("Subsample validation set: ", args.val_subsample)
    ...
    result = subprocess.run(['nvidia-smi', '--query-gpu=memory.used', '--format=csv'], stdout=subprocess.PIPE, universal_newlines=True)
    mem_usage = int(result.stdout.strip().split('\n')[1].split()[0])
    print(f"Final memory usage: {mem_usage} MB")
