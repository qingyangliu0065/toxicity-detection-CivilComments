...
class LossComputer:
    ...
    def log_stats(self, logger, is_training):
        ...
        if not is_training:
            logger.write(f"Spurious Score = {spurious_score:.6f}\n")
            logger.write(f"Weighted Spurious Score = {spurious_score_with_size_weight:.6f}\n")
            if self.dataset_name == "jigsaw":
                logger.write(f"Our Spurious Score = {our_spurious_score}\n")
                logger.write(f"Our Weighted Spurious Score = {our_weighted_spurious}\n")
            logger.write(f"Average Accuracy Divided by Std: {average_divided_by_std:.4f}\n")
            logger.write(f"Average Accuracy Divided by Std Times Worst-Group: {average_divided_by_std_times_worst:.4f}\n")
            if self.dataset_name != "MultiNLI":
                # logger.write(f"Class One = {num_00s:.4f} + {num_01s:.4f} divided by {num_11s:.4f}")
                logger.write(f"Class One Accuracy: {class_one_acc:.4f}\n")
                logger.write(f"Class Two Accuracy: {class_two_acc:.4f}\n")
                logger.write(f"Class Accuracy Absolute Difference: {abs_difference_bt_class_acc:.4f}\n")
            else:
                logger.write(f"Class One Accuracy: {class_one_acc:.4f}\n")
                logger.write(f"Class Two Accuracy: {class_two_acc:.4f}\n")
                logger.write(f"Class Third Accuracy: {class_third_acc:.4f}\n")
        logger.flush()  
