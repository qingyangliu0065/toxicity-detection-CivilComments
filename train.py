...
def train(
    ...
    if args.method == 'AUX2':
        ...
        best_test_worst_group_acc = 0.0 
        best_test_worst_group_epoch = 0
        ...
        highest_validation_avg_corresponding_test_worst = 0 
        highest_validation_avg_corresponding_test_avg = 0 
        highest_validation_avg_corresponding_epoch = 0
    ...
    while True:
        ...
        result = subprocess.run(['nvidia-smi', '--query-gpu=memory.used', '--format=csv'], stdout=subprocess.PIPE, universal_newlines=True)
        mem_usage = int(result.stdout.strip().split('\n')[1].split()[0])
        print(f"Start of Epoch memory usage: {mem_usage} MB")
        ...
        if args.method == 'AUX2':
            ...
            if curr_average_acc > best_average_acc:
                best_average_acc = curr_average_acc
                best_average_acc_epoch = epoch 
            logger.write(f'Best worst-group accuracy: {best_worst_group_acc_val}\n')
            logger.write(f'Best worst-group epoch: {best_worst_group_epoch_val}\n')
            
            logger.write(f'Smallest absolute class difference: {best_class_diff}\n')
            logger.write(f'Smallest absolute class difference epoch: {best_class_diff_epoch}\n')

            logger.write(f'Best Validation Average Accuracy: {best_average_acc}\n')
            logger.write(f'Best Validation Average Accuracy Epoch: {best_average_acc_epoch}\n')
        ...
        if dataset["test_data"] is not None:
            ...
            spurious_score_cur, weighted_score_cur, curr_worst_test_group_acc, curr_test_class_difference, test_curr_avg_acc = run_epoch(
                epoch,
                model,
                optimizer,
                dataset["test_loader"],
            ...
            if args.method == 'AUX1':
                ...
                logger.write(f'Current best test spurious score epoch: {best_test_epoch}\n')
                logger.write(f'Current best test weighted spurious score epoch: {best_test_weighted_epoch}\n')
        

            if args.method == 'AUX2':
                if curr_worst_test_group_acc > best_test_worst_group_acc:
                    best_test_worst_group_acc = curr_worst_test_group_acc
                    best_test_worst_group_epoch = epoch  
                logger.write(f'Best worst-group accuracy: {best_test_worst_group_acc}\n')
                logger.write(f'Best worst-group epoch: {best_test_worst_group_epoch}\n')
                ...
                if epoch == best_average_acc_epoch:
                    highest_validation_avg_corresponding_test_worst = curr_worst_test_group_acc
                    highest_validation_avg_corresponding_test_avg = test_curr_avg_acc
                    highest_validation_avg_corresponding_epoch = epoch 
                logger.write(f'\nBest Stats Info:\n')
                ...
                logger.write(f'Highest Validation Average Accuracy: {best_average_acc}\n')
                logger.write(f'Highest Validation Average Corresponding Test Worst-group Acc: {highest_validation_avg_corresponding_test_worst}\n')
                logger.write(f'Highest Validation Average Corresponding Test Average Acc: {highest_validation_avg_corresponding_test_avg}\n')
                logger.write(f'Highest Validation Average Corresponding Epoch: {highest_validation_avg_corresponding_epoch}\n')
        ...
        #     print("End of Epoch Memory Usage: ", tracemalloc.get_traced_memory())
        result = subprocess.run(['nvidia-smi', '--query-gpu=memory.used', '--format=csv'], stdout=subprocess.PIPE, universal_newlines=True)
        mem_usage = int(result.stdout.strip().split('\n')[1].split()[0])
        print(f"End of Epoch memory usage: {mem_usage} MB")
        
        end_now = datetime.now()
        current_end_time = end_now.strftime("%H:%M:%S")
        print("End of Epoch: Current Time =", current_end_time)
