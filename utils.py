...
from transformers import BBertModel

class Bert_lstm(nn.Module):
    USE_CUDA = True 
    def __init__(self, bertpath, hidden_dim, output_size,n_layers=2,bidirectional=True, drop_prob=0.5):
    ...
    def forward(self, x, hidden):
        ...
        if self.bidirectional:
            ...
            #print(hidden_last_out.shape,'hidden_last_out')   
        else:
...
def get_model(model, pretrained, resume, n_classes, dataset, log_dir, args, 
    ...
    if args.bertLstm:
        model = Bert_lstm(bertpath="saved_bert_model.pth", hidden_dim=args.lstm_hidden, output_size=2)
